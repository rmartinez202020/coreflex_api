...
def start_graphic_stream(
    *,
    ...
    payload = {
        ...
        "sourceMode": str(source_mode or "local").strip().lower(),
        "gatewayId": str(gateway_id or "").strip(),
        "gatewayTailscaleIp": str(gateway_tailscale_ip or "").strip(),
        "gatewayLocalDeviceIp": str(gateway_local_device_ip or "").strip(),
        "gatewayStatus": str(gateway_status or "").strip().lower(),
        "macAddress": str(mac_address or "").strip(),
    ...
    _dbg(
        ...
        gateway_tailscale_ip=payload["gatewayTailscaleIp"],
        gateway_local_device_ip=payload["gatewayLocalDeviceIp"],
